      ...
      try {
        ...
        if (res.ok) {
          ...
          const myCourses = coursesList.filter((c: any) => 
            c.teacherId === user?.id || c.teacher?.username === user?.username
          );
  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">{t.dashboard?.welcome} {user?.name || 'Teacher'}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="stats shadow bg-base-100 border border-base-200">
          <div className="stat">
            ...
            <div className="stat-desc">{t.dashboard?.activeSessions}</div>
          </div>
        </div>
        ...
        <div className="stats shadow bg-base-100 border border-base-200">
          <div className="stat">
            <div className="stat-figure text-secondary">
              <FontAwesomeIcon icon={faCalendarAlt} className="w-8 h-8" />
            </div>
            <div className="stat-title">{t.dashboard?.todaySessions}</div>
            <div className="stat-value text-secondary">{todaySchedule.length}</div>
            <div className="stat-desc">{t.dashboard?.thisWeek}</div>
          </div>
        </div>
        ...
        <div className="stats shadow bg-base-100 border border-base-200">
          <div className="stat">
            ...
            <div className="stat-title">{t.dashboard?.totalStudents}</div>
        ...
        <div className="lg:col-span-2 card bg-base-100 shadow-xl border border-base-200">
          <div className="card-body">
            <h2 className="card-title flex justify-between">
              ...
              <Link href="/schedule" className="btn btn-ghost btn-xs">{t.common?.view}</Link>
            </h2>
                  ...
                  {todaySchedule.length > 0 ? (
                    todaySchedule.map((slot, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <Link 
                            href={`/attendance?courseId=${slot.courseId}`} 
            ...
            <div className="flex flex-col gap-3 mt-4">
              {courses.slice(0, 5).map((course) => (
                <div key={course.id} className="flex justify-between items-center p-3 bg-base-200 rounded-lg">
                  ...
                  <div className="badge badge-ghost">{course.enrolledStudents?.length || 0}</div>
                </div>
              ))}
              {courses.length === 0 && (
