                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!readOnly && session.status === 'closed' && (
                       <button
                         ...
                         onClick={async () => {
                            try {
                              setClosingSession(session.id); // Re-use closing state for loading spinner
                              ...
                              if (result.success) {
                                showToast({ message: t.attendanceManagement.openSessionSuccess || 'Session re-opened successfully', type: 'success' });
                                onSessionClosed(); // Refresh list (naming is weird but it triggers refresh)
                              } else {
                                throw new Error(result.error);
                              }
                            } catch (error) {
                         ...
                         disabled={closingSession === session.id}
                         title={t.attendanceManagement.reopen || 'เปิดใช้งานอีกครั้ง'}
                       >
                         {closingSession === session.id ? (
                           <span className="loading loading-spinner loading-xs"></span>
                         ) : (
