...
import { faUpload, faCheckCircle, faExclamationTriangle, faTrash, faBolt, faShieldAlt } from '@fortawesome/free-solid-svg-icons';
import Image from 'next/image';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type FaceAPIModule = any;

type RegistrationMode = 'single' | 'multi';
...
export default function FaceUpload({
  ...
  const [registrationMode, setRegistrationMode] = useState<RegistrationMode>('multi');
  ...
  const captureSteps = useMemo(() => {
    ...
    return allSteps;
  }, [registrationMode, allSteps]);

  const hasStartedCapture = currentStepIndex > 0 || descriptors.length > 0;
    ...
    <div className="space-y-6">
       ...
       <div className="flex justify-center gap-2 mb-4">
         <button
           ...
           disabled={isComplete}
           className={`btn btn-sm gap-2 ${registrationMode === 'single' 
             ? 'btn-primary' 
             : 'btn-outline btn-ghost'} ${isComplete ? 'btn-disabled opacity-60' : ''}`}
         >
           <FontAwesomeIcon icon={faBolt} />
           {t.register.faceSteps?.singleMode || 'Quick (1 Image)'}
         </button>
         <button
           type="button"
           onClick={() => handleModeChange('multi')}
           disabled={isComplete}
           className={`btn btn-sm gap-2 ${registrationMode === 'multi' 
             ? 'btn-primary' 
             : 'btn-outline btn-ghost'} ${isComplete ? 'btn-disabled opacity-60' : ''}`}
         >
           <FontAwesomeIcon icon={faShieldAlt} />
           {t.register.faceSteps?.multiMode || 'High Accuracy (5 Images)'}
         </button>
       </div>
                    ...
                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                        <div className="badge badge-success gap-2 p-3">
                            ...
                            {registrationMode === 'single' 
                              ? (t.register.faceSteps.completedSingle || t.register.faceSteps.completed)
