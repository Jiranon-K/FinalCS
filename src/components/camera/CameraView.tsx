...
export default function CameraView({
  ...
  const [recognizedPersons, setRecognizedPersons] = useState<
    Map<number, { name: string; confidence: number; id?: string }>
  >(new Map());
  const [knownPersons, setKnownPersons] = useState<PersonForRecognition[]>([]);

  const [lastAttendanceRecord, setLastAttendanceRecord] = useState<
    Map<string, Date>
  >(new Map());
  ...
  const { modelsLoaded, detectFaces, recognizeFace, settings, updateSettings } =
    useFaceAPI();
  ...
  const recordAttendance = async (personId: string, personName: string) => {
    console.log("Attempting to record attendance:", {
      personId,
      personName,
    ...
    setLastAttendanceRecord((prev) =>
      new Map(prev).set(personId, new Date(now)),
    );
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.isNewCheckIn) {
            showToast({
              type: "success",
              message: `✅ ${t.attendanceManagement?.recordSuccess || "บันทึกการเข้าเรียนสำเร็จ"}: ${personName}`,
            });
          }
  ...
  const handleLivenessFailed = () => {
    showToast({
      type: "error",
      message: t.liveness?.failed || "Verification Failed",
    });
    setShowLivenessCheck(false);
    setPendingPerson(null);
  };
    ...
    const runDetection = async () => {
      ...
      try {
        ...
        const recognitionMap = new Map<
          number,
          { name: string; confidence: number; id?: string }
        >();
        ...
        for (let i = 0; i < faces.length; i++) {
          ...
          if (face.descriptor && knownPersons.length > 0) {
            ...
            if (match) {
              ...
              const MIN_CONFIDENCE_FOR_RECORD = 0.35;
              const isMatchConfident =
                match.confidence >= 1 - settings.recognitionThreshold &&
                match.confidence >= MIN_CONFIDENCE_FOR_RECORD;

              if (isMatchConfident && match.personId) {
                const alreadyCheckedIn = lastAttendanceRecord.has(
                  match.personId,
                );
        ...
        if (ctx) {
          ...
          if (faces.length === 0) {
            ...
            const guideText =
              t.camera?.positionFace || "Position your face here";
            ctx.fillText(guideText, cx, cy + ry + 28);
            ctx.textAlign = "start";
            ctx.restore();
          }
          ...
          faces.forEach((face, index) => {
            ...
            const isKnown =
              recognized && recognized.name !== (t.camera.unknown || "Unknown");
            ...
            const confidenceValue = isKnown
              ? recognized.confidence
              : face.detection.score;
            if (confidenceValue > 0) {
              const barHeight = 5;
              const barY = box.y - labelHeight - 5 - barHeight - 3;
              const barWidth = textWidth + padding * 2;

              ctx.fillStyle = "rgba(0, 0, 0, 0.35)";
              ctx.beginPath();
              ctx.roundRect(box.x, barY, barWidth, barHeight, 3);
              ctx.fill();

              const fillWidth = barWidth * Math.min(confidenceValue, 1);
              ctx.fillStyle = isCheckedIn
                ? "#60a5fa"
                : isKnown
                  ? "#34d399"
                  : "#f87171";
              ctx.beginPath();
              ctx.roundRect(box.x, barY, fillWidth, barHeight, 3);
              ctx.fill();
            }
        ...
        <div className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto justify-end">
            <div className="dropdown dropdown-end">
                ...
                <div className="flex flex-col items-start text-left overflow-hidden w-full mr-2">
                  <span className="font-bold text-xs truncate w-full">
                    {settings.detectorModel === "ssd_mobilenetv1" &&
                      t.faceSettings.ssdMobilenetName}
                    {settings.detectorModel === "tiny_face_detector" &&
                      t.faceSettings.tinyFaceDetectorName}
                    {settings.detectorModel === "mediapipe" &&
                      (t.faceSettings?.mediapipeName || "MediaPipe (Google)")}
                  </span>
                  <span className="text-[10px] opacity-70 truncate w-full">
                    {settings.detectorModel === "ssd_mobilenetv1" &&
                      t.faceSettings.ssdMobilenetDesc}
                    {settings.detectorModel === "tiny_face_detector" &&
                      t.faceSettings.tinyFaceDetectorDesc}
                    {settings.detectorModel === "mediapipe" &&
                      (t.faceSettings?.mediapipeDesc ||
                        "Fast & accurate (Google AI)")}
                  </span>
                </div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <ul
                tabIndex={0}
                className="dropdown-content z-1 menu p-2 shadow-lg bg-base-100 rounded-box w-64 mt-1 border border-base-content/10"
              >
                <li>
                  <button
                    onClick={() => {
                  ...
                  >
                    ...
                    <span className="text-[10px] opacity-70 truncate block w-full">
                      {t.faceSettings.ssdMobilenetDesc}
                    </span>
                  </button>
                </li>
                ...
                <li>
                  <button
                    onClick={() => {
                      updateSettings({ detectorModel: "mediapipe" });
                      const elem = document.activeElement as HTMLElement;
                      if (elem) elem.blur();
                    }}
                    className={`flex flex-col items-start gap-0.5 py-2 ${settings.detectorModel === "mediapipe" ? "active" : ""}`}
                  >
                    <span className="font-bold text-xs">
                      {t.faceSettings?.mediapipeName || "MediaPipe (Google)"}
                    </span>
                ...
                <li className="px-2 py-2">
                  <div className="flex flex-col gap-2 p-0 active:bg-transparent hover:bg-transparent cursor-default">
                    <div className="flex items-center justify-between w-full">
                      <span className="font-bold text-xs">
                        {t.faceSettings?.recognitionThreshold ||
                    ...
                    />
                    <div className="flex justify-between w-full text-[10px] opacity-50 px-0.5">
                      <span>{t.faceSettings?.strict || "Strict"}</span>
                      <span>{t.faceSettings?.loose || "Loose"}</span>
                    </div>
                    <span className="text-[10px] opacity-70">
                      {t.faceSettings?.thresholdHelp ||
                        "Lower value = Stricter check"}
                    </span>

                    <details className="w-full">
                      <summary className="text-[10px] opacity-60 cursor-pointer hover:opacity-90 flex items-center gap-1 select-none transition-opacity">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 20 20"
                          fill="currentColor"
                        ...
                        <span>
                          {t.faceSettings?.thresholdTipTitle ||
                            "Why lower = stricter?"}
                        </span>
                      </summary>
                      <div className="mt-1.5 ml-4 space-y-1 text-[10px] opacity-60 leading-relaxed">
                        <p>
                          {t.faceSettings?.thresholdTipBody ||
                            'The system measures face "distance" — 0 = perfect match, higher = more different. So a lower threshold means less difference is allowed = stricter.'}
                        </p>
                        <div className="flex flex-col gap-0.5 pt-0.5">
                          <span>
                            {t.faceSettings?.thresholdSafe ||
                              "🔒 Low value = High security (must match closely)"}
                          </span>
                          <span>
                            {t.faceSettings?.thresholdFast ||
                              "⚡ High value = Faster (more flexible)"}
                          </span>
                        </div>
                      </div>
                    </details>
              ...
              >
                {devices.map((device, index) => (
                  <li key={device.deviceId}>
                    ...
                    >
                      <span className="font-bold text-xs truncate block w-full">
                        {device.label}
                      </span>
                      <span className="text-[10px] opacity-70 truncate block w-full">
        ...
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-base-200/50 backdrop-blur-sm z-10">
            <Loading
              variant="spinner"
              size="lg"
              text={t.camera.initializing || "Initializing Camera..."}
            />
          </div>
        )}
            ...
            <div className="text-center space-y-6">
              <div className="relative inline-block">
                <svg
                  ...
                  strokeWidth={1}
                  stroke="currentColor"
                  className="w-32 h-32 text-base-content/20"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ...
                  {detectedFaces.length > 0 && recognizedPersons.size > 0 && (
                    <>
                      <div className="flex items-center gap-2 bg-primary/20 backdrop-blur-sm text-primary-content px-3 py-1.5 rounded-lg border border-primary/20 w-fit">
                        ...
                        <span className="text-xs font-bold">
                          {
                            Array.from(recognizedPersons.values()).filter(
                              (p) => p.name !== "Unknown",
                            ).length
                          }{" "}
                          {t.camera.known}
