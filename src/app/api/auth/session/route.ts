...
export async function GET() {
  try {
    ...
    let hasProfileRegistered = !!user.profileId;

    if (user.role === 'student' && hasProfileRegistered) {
      const studentProfile = profile as any;
      if (!studentProfile?.faceDescriptors || studentProfile.faceDescriptors.length === 0) {
