...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: RecordAttendanceRequest & { token?: string } = await request.json();
    const { studentId, sessionId, timestamp, confidence, method, token } = body;
    ...
    if (token) {
      ...
      if (!verified) {
        return badRequestResponse('Invalid attendance token');
      }
      if (verified.studentId !== studentId || verified.sessionId !== sessionId) {
        return badRequestResponse('Token mismatch');
      }
    }
