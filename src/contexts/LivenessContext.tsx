...
const LivenessContext = createContext<LivenessContextType | undefined>(
  undefined,
);
...
export function LivenessProvider({ children }: { children: React.ReactNode }) {
    ...
    (landmarks: Array<{ x: number; y: number }>) => {
      ...
      setState((prev) => {
        ...
        if (prev.currentChallenge.action === "BLINK") {
          ...
          prev.currentChallenge.action === "TURN_LEFT" ||
          prev.currentChallenge.action === "TURN_RIGHT"
        ) {
        ...
        if (isActionCompleted && prev.currentChallenge) {
          ...
          updatedChallenges[prev.currentChallengeIndex] =
  ...
  const updateSettings = useCallback(
    (newSettings: Partial<LivenessSettings>) => {
...
export function useLivenessContext() {
  ...
  if (context === undefined) {
    throw new Error(
      "useLivenessContext must be used within a LivenessProvider",
    );
  }
  return context;
}
