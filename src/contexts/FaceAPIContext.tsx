...
export function FaceAPIProvider({ children }: { children: React.ReactNode }) {
  ...
  const [settings, setSettings] =
    useState<FaceRecognitionSettings>(DEFAULT_SETTINGS);
  const [error, setError] = useState<string | null>(null);

  const faceapiRef = useRef<FaceAPIModule | null>(null);
  const tfRef = useRef<TFModule | null>(null);
  const mediapipeReadyRef = useRef<boolean>(false);
  ...
  const loadModels = useCallback(async () => {
    ...
    try {
      ...
      if (settings.enableAgeGender) {
        modelsToLoad.push(faceapi.nets.ageGenderNet.loadFromUri(modelPath));
      }
      ...
      // If MediaPipe is selected, also initialize it
      if (settings.detectorModel === "mediapipe") {
        await initMediaPipeFaceDetector(settings.detectionThreshold);
      ...
      try {
        if (
          ...
          if (!(input instanceof HTMLVideoElement)) {
            throw new Error(
              "MediaPipe detector only supports HTMLVideoElement input.",
            );
          }

          const timestampMs = performance.now();
          const mpFaces = detectFacesMediaPipe(input, timestampMs);

          const resultsWithDescriptors: FaceDetectionResult[] = [];

          for (const mpFace of mpFaces) {
            try {
              const { box } = mpFace.detection;
              ...
              const cropW = Math.min(
                input.videoWidth - cropX,
                box.width + padding * 2,
              );
              const cropH = Math.min(
                input.videoHeight - cropY,
              ...
              if (cropCtx) {
                ...
                if (faceapiDetection) {
                  mpFace.descriptor = faceapiDetection.descriptor;
                  if (faceapiDetection.landmarks) {
                    mpFace.landmarks = {
                      positions: faceapiDetection.landmarks.positions.map(
                        (pos: { x: number; y: number }) => ({
                          x: pos.x + cropX,
                          y: pos.y + cropY,
                        }),
                      ),
                      shift: faceapiDetection.landmarks.shift || { x: 0, y: 0 },
                    };
                  }
                }
              }
              ...
              console.warn(
                "Failed to extract descriptor for MediaPipe face:",
                descErr,
              );
            }

            resultsWithDescriptors.push(mpFace);
          }

          return resultsWithDescriptors;
        }
        ...
        if (settings.enableExpressions) {
          detections = (
            detections as { withFaceExpressions: () => unknown }
          ).withFaceExpressions();
        }

        if (settings.enableAgeGender) {
          detections = (
            detections as { withAgeAndGender: () => unknown }
          ).withAgeAndGender();
        }

        const results: FaceDetectionResult[] = (
          (await (detections as Promise<unknown>)) as FaceApiDetection[]
        ).map((detection) => {
        ...
        if (tf) {
          ...
          if (memInfo.numTensors > 500) {
            console.warn(
              `High tensor count: ${memInfo.numTensors}. Potential memory leak.`,
            );
          }
        }
      }
  ...
  const updateSettings = useCallback(
    (newSettings: Partial<FaceRecognitionSettings>) => {
      setSettings((prev) => ({
        ...prev,
        ...newSettings,
      }));
      ...
      if (newSettings.detectorModel) {
        if (newSettings.detectorModel !== "mediapipe") {
          disposeMediaPipeFaceDetector();
          mediapipeReadyRef.current = false;
        }
