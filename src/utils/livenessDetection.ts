...
export function extractEyeLandmarks(
...
): { leftEye: Point[]; rightEye: Point[] } | null {
  const maxIndex = Math.max(
    ...EYE_LANDMARKS.LEFT_EYE,
    ...EYE_LANDMARKS.RIGHT_EYE,
  );
  if (!landmarks || landmarks.length <= maxIndex) {
    return null;
  }
  ...
  if (leftEye.some((p) => !p) || rightEye.some((p) => !p)) {
    return null;
  }
...
export function calculateAverageEAR(
  ...
  const eyes = extractEyeLandmarks(landmarks);
  if (!eyes) {
    return 0.3;
  }

  const leftEAR = calculateEAR(eyes.leftEye);
  const rightEAR = calculateEAR(eyes.rightEye);
...
export class HeadPoseDetector {
  ...
  static detect(landmarks: Array<{ x: number; y: number }>): {
    ...
    const maxIndex = Math.max(
      ...
      HEAD_POSE_LANDMARKS.LEFT_EYE_OUTER,
      HEAD_POSE_LANDMARKS.RIGHT_EYE_OUTER,
    );
    if (!landmarks || landmarks.length <= maxIndex) {
